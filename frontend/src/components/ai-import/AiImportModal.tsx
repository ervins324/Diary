...
export function AiImportModal({ isOpen, onClose }: AiImportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 md:p-6 overflow-y-auto">
      <div className="bg-bg-secondary w-full max-w-5xl rounded-xl shadow-lg border border-border flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex justify-between items-center p-4 border-b border-border">
          <h2 className="text-xl font-semibold text-text-primary">Import Schedule via AI</h2>
          ...
          {parsedData.length === 0 ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full py-10">
              ...
              <button
                ...
                className="mt-6 w-full py-2 px-4 bg-accent hover:bg-accent/90 text-white rounded-md font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
              >
                {parseMutation.isPending && <Loader2 size={18} className="animate-spin" />}
                {parseMutation.isPending ? 'Parsing Image...' : 'Parse Schedule'}
              </button>
            </div>
          ) : (
            <>
              ...
              <div className="w-full md:w-1/3 flex flex-col gap-4">
                <h3 className="font-medium text-text-primary">Source Image</h3>
                <div className="bg-bg-tertiary rounded-lg border border-border overflow-hidden h-64 md:h-full flex items-center justify-center">
                  {previewUrl && (
                    <img src={previewUrl} alt="Schedule source" className="max-w-full max-h-full object-contain" />
                  )}
                </div>
              </div>
              ...
              <div className="w-full md:w-2/3 flex flex-col gap-4">
                <div className="flex justify-between items-center">
                  <h3 className="font-medium text-text-primary">Review & Edit</h3>
                  {/* Week type selector */}
                  <select 
                    value={weekType}
                  ...
                  >
                    <option value="numerator">Numerator Week</option>
                    <option value="denominator">Denominator Week</option>
                    <option value="both">Both Weeks</option>
                  </select>
                </div>
                
                <div className="flex-1 border border-border rounded-lg overflow-hidden bg-bg-primary max-h-[50vh] md:max-h-full">
        ...
        {parsedData.length > 0 && (
          <div className="p-4 border-t border-border flex justify-end gap-3 bg-bg-tertiary/50">
            <button 
            ...
            <button
              ...
              className="px-6 py-2 bg-accent hover:bg-accent/90 text-white rounded-md font-medium text-sm transition-colors disabled:opacity-50 flex items-center gap-2"
            >
